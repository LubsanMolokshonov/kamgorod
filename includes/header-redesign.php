...
<head>
  ...
  <title><?php echo htmlspecialchars($pageTitle ?? 'ФГОС-Практикум', ENT_QUOTES, 'UTF-8'); ?></title>
...
<?php if (empty($ogImage)) $ogImage = SITE_URL . '/assets/images/og-home.jpg'; ?>
  ...
  <script type="text/javascript">
    ...
    ym(106465857,"init",{clickmap:true,trackLinks:true,accurateTrackBounce:true,webvisor:true,ecommerce:"dataLayer"});
  </script>
  ...
  <script type="text/javascript">
    ...
    ymab('metrika.106465857','init');
  </script>

<?php if (isset($earlyHeadScripts)): ?>
  <?php foreach ($earlyHeadScripts as $script): echo $script; endforeach; ?>
<?php endif; ?>
...
<?php
$allJsonLd = [];
if (!empty($jsonLdArray)) $allJsonLd = $jsonLdArray;
elseif (!empty($jsonLd)) $allJsonLd = [$jsonLd];
if (!empty($breadcrumbJsonLd)) $allJsonLd[] = $breadcrumbJsonLd;
