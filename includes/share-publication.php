...
// Pre-filled текст: для Telegram/WhatsApp/native URL передаётся внутри текста,
// поэтому текст собирается под каждый канал со «своей» меткой.
...
$urlTg     = $shareUrlFor('telegram');
$urlWa     = $shareUrlFor('whatsapp');
$urlOk     = $shareUrlFor('ok');
...
<div class="share-buttons pub-share"
  ...
  <a href="https://t.me/share/url?url=<?php echo urlencode($urlTg); ?>&text=<?php echo urlencode($shareTextFor($urlTg)); ?>" target="_blank" rel="noopener" class="share-btn telegram" data-network="telegram" title="Telegram">TG</a>
  <a href="https://api.whatsapp.com/send?text=<?php echo urlencode($shareTextFor($urlWa)); ?>" target="_blank" rel="noopener" class="share-btn whatsapp" data-network="whatsapp" title="WhatsApp">WA</a>
  <a href="https://connect.ok.ru/offer?url=<?php echo urlencode($urlOk); ?>&title=<?php echo $encTitle; ?>" target="_blank" rel="noopener" class="share-btn ok" data-network="ok" title="Одноклассники">OK</a>
