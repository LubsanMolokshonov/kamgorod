...
class MaterialGenerator
{
    ...
    private function executeGeneration(int $generationId, array $type, array $params, string $mode, ?int $userId, ?string $funnelSessionId, ?int $chargeTxnId, int $tokenCost): array
    {
        ...
        try {
            ...
            // Ступень прокидываем в данные рендера служебным ключом (_*, не выводится),
            // чтобы рендер мог, например, не показывать «Домашнее задание» для ДОО.
            $aiData['_stage'] = MaterialType::deriveStage(
                (string)($params['class'] ?? ''),
            ...
            // 5b. Иллюстрации для слайдов презентации (главная жалоба методистов —
            //     «презентация без картинок»). Инфраструктура PPTX уже умеет рисовать картинку
            //     справа на слайде (поле _image_abs), но её никто не заполнял. Генерируем через
            //     YandexART для содержательных слайдов, у которых модель вернула image_prompt.
            //     Best-effort: при любой ошибке слайд просто остаётся текстовым, генерацию не валим.
            //     Только full-режим (в превью файл не рендерим и картинки не нужны).
            if (!$isPreview && $typeSlug === 'prezentatsiya' && !empty($aiData['slides']) && is_array($aiData['slides'])) {
                try {
                    $this->attachSlideImages($aiData['slides'], $slug, $params);
                } catch (Throwable $slideImgErr) {
                    error_log('MaterialGenerator: slide images failed (non-fatal): ' . $slideImgErr->getMessage());
                }
            }
...
4. КЛЮЧИ И ВОПРОСЫ ТЕСТА: каждый правильный ответ должен реально присутствовать среди вариантов; слова-эталоны должны существовать в русском языке и быть посильны классу; для вопроса с type=single правильный ответ строго ОДИН (если правильных несколько — поставь type=multiple). Проверь, что пропущенная буква/звук в задании совпадает с ключом. ОЦЕНИВАНИЕ ТЕСТА: у каждого вопроса должно быть указано число баллов; сумма баллов всех вопросов ДОЛЖНА равняться note.max_score, а шкала (note.scale) — соответствовать этой сумме. Если не сходится — пересчитай. КАЖДЫЙ вопрос выбора (single/multiple) ОБЯЗАН иметь непустой массив options (минимум 3 варианта); если вариантов нет — сгенерируй их. У КАЖДОГО открытого вопроса (open) в answer_key — критерии начисления баллов (например, «3 верных признака — 2 балла, 2 — 1 балл»). СТРУКТУРА ПО БЛОКАМ: расставь вопросы от простого к сложному и проставь block: «А» (выбор) → «В» (краткий ответ) → «С» (развёрнутый), не перемешивай. ДИСТРАКТОРЫ корректны и однозначны: НЕ дублируй проверяемое понятие в вариантах (на вопрос «какое время года после лета?» среди вариантов НЕ должно быть «Лето»); неверные варианты должны быть ОДНОЗНАЧНО неверными (не используй «снег осенью» как заведомо неверный — для многих регионов это норма; бери однозначный маркер). Для начальной школы (1-2 класс) не злоупотребляй развёрнутыми ПИСЬМЕННЫМИ ответами (блок С) — у первоклассника письмо только формируется; такие задания делай необязательными/устными или заменяй выбором.
...
7. СТУПЕНЬ ДО (ДЕТСКИЙ САД) — ЭТО НЕ ШКОЛА: если ступень {$stage} — дошкольная, ПЕРЕДЕЛАЙ материал под ФГОС ДО/ФОП ДО: пиши «воспитатель»/«дети» (не «учитель»/«ученик»); основа — ИГРА (сюжет, персонаж, сюрпризный момент), наглядность и предметная деятельность (не доска и запись); вместо «УУД» — «целевые ориентиры»/«педагогическое наблюдение»; укажи интеграцию образовательных областей ФОП ДО; УБЕРИ домашнее задание (нарушение СанПиН), балльные отметки, абстрактные вопросы не по возрасту, чтение/письмо детьми, если возраст этого не предполагает. Для речевого развития не подменяй его познавательным (классификация — это познавательное).
...
14. «ЗВЁЗДОЧКА»/ПОВЫШЕННЫЙ УРОВЕНЬ (level="advanced") должна быть РЕАЛЬНО сложнее базовых заданий (анализ/применение/нестандартность, а не просто «выбери сказку») и иметь в инструкции/ключе пояснение, что это задание повышенного уровня и как оно оценивается.
15. ТЕХ.КАРТА/КОНСПЕКТ: ЦЕЛЬ — одна, формулируется как цель («создать условия для…», «организовать…»), НЕ списком задач; задачи — отдельно. Укажи учебный модуль/раздел и межпредметные связи, если уместно. Планируемые результаты (предметные) и УУД должны быть представлены явно.
16. КЛАССНЫЙ ЧАС ДЛЯ ПОДРОСТКОВ (ООО/СОО): без «детского» поучающего тона; обязательны проблемные вопросы, дискуссия/дебаты/разбор кейсов, распределение ролей в группах (спикер, тайм-менеджер и т.п.), внутренняя проблематизация и АКТУАЛЬНЫЕ реалии (вейпы/поды, энергетики, зависимость от коротких видео, номофобия, кибербуллинг) вместо общих формулировок; добавь обучение навыкам (сказать «нет», справиться со стрессом).
17. ПРЕЗЕНТАЦИЯ: каждый содержательный слайд — 3–5 КОРОТКИХ буллетов (тезисы), без сплошного текста и без переноса всей лекции на слайд; у содержательных слайдов заполни поле image_prompt (описание иллюстрации на русском, без текста на картинке); титульный и итоговый слайды image_prompt можно не заполнять.
...
TXT;
    ...
    /**
     * Догенерировать иллюстрации для слайдов презентации (YandexART) и проставить пути:
     *   - _image_abs — абсолютный путь к файлу (для PptxRenderer, рисующего картинку на слайде);
     *   - _image_url — относительный URL «/uploads/…» (для HTML-превью и PDF-фолбэка).
     * Изображения генерируем только для содержательных слайдов с непустым image_prompt и не более
     * MATERIAL_SLIDE_IMAGES_MAX штук (по умолчанию 6) — чтобы не раздувать время и расход на ИИ.
     * Меняет массив $slides по ссылке. Best-effort: молча пропускает слайды, которым картинку
     * не удалось получить.
     *
     * @param array<int,array> $slides слайды из ai_output (по ссылке)
     */
    private function attachSlideImages(array &$slides, string $slug, array $params): void
    {
        $art = new YandexArtService();
        if (!$art->isEnabled()) {
            return;
        }
        $max = defined('MATERIAL_SLIDE_IMAGES_MAX') ? (int)MATERIAL_SLIDE_IMAGES_MAX : 6;
        if ($max <= 0) {
            return;
        }
        ...
        foreach ($slides as $i => &$slide) {
            ...
            $imgPrompt = trim((string)($slide['image_prompt'] ?? ''));
            if ($imgPrompt === '') {
                continue;
            }
            // Подстраховка стиля: предметная иллюстрация, без текста на картинке.
            $fullPrompt = $imgPrompt
                . ($subject !== '' ? ', предмет ' . $subject : '')
                . ', реалистичная образовательная иллюстрация, без текста и подписей на картинке';
            $rel = $art->generateAndStore($fullPrompt, $slug . '-s' . ((int)$i + 1), '4:3');
            if ($rel === null) {
                continue;
            }
            ...
            $slide['_image_abs'] = $abs;
            $slide['_image_url'] = '/' . ltrim($rel, '/');
            $made++;
        }
        unset($slide);
    }
