...
- `in_progress` — взятые в работу ранее (часто старый «хвост»); трогай их, только если пользователь попросил «разобрать все».
  Разбирая их, **перепроверь сам статус**: если действие за нами не висит (ждём только ответа человека
  или всё уже сделано) — переведи в `resolved`/`closed`, а не оставляй висеть.
...
#### Какой `verdict` ставить (это и есть итоговый статус обращения)

Статус меняет сам скрипт по полю `verdict` в кейсе. `ENUM('new','in_progress','resolved','closed')`.

| verdict | статус | когда |
|---|---|---|
| `resolve` | `resolved` | **по умолчанию.** Мы сделали, что просили, или дали полный ответ. С нашей стороны не висит ни одного действия. |
| `in_progress` | `in_progress` | **только если конкретное действие за НАМИ** и оно ещё не выполнено: учебный отдел готовит документ, бухгалтерия подтверждает возврат, ждём трек-номер. Обязательно назови в `note` — кто исполнитель и чего именно ждём. |
| `close` | `closed` | действовать не нужно и не по адресу: спам, дубль, вопрос не к нам, человек сам отозвал просьбу. |

Главное правило: **«ждём ответа пользователя» — это `resolve`, а не `in_progress`.** Мяч на стороне человека;
если он ответит, придёт новое обращение и цепочка возобновится. Иначе обращения копятся в `in_progress`
годами и каждый прогон перечитывает их заново. Так и вышло с «хвостом» из 7 штук к 01.09.2026.

`in_progress` без названного исполнителя и без ожидаемого действия — ошибка. Не оставляй обращение
в этом статусе просто потому, что «не уверен».
...
**Сначала — итоговую строку боевого запуска.** Скрипт печатает `=== Итого: писем=N, отправлено=S, ошибок=E ===`.

- `E > 0` — обращение **НЕ отвечено**, чем бы ни закончился DRY-RUN. Разбери каждую строку `[ERR]`,
  исправь причину и перезапусти только упавшие кейсы (`--send --only=<буква>`).
- Никогда не считай кейс закрытым по факту создания записи в `alert_messages` — записи пишутся и при
  неудачной отправке. Единственное доказательство доставки — `unisender_id` в строке `[OK]`.
- Если причину не удалось устранить, кейс идёт в отчёт **отдельным блоком «НЕ ДОСТАВЛЕНО»** с текстом ошибки
  и тем, что нужно сделать человеку. Не смешивай его с «ждём ответа пользователя»: человек письма не видел.
...
```
Скрипт ставит статус по `verdict` (`resolve`→`resolved`, `close`→`closed`, иначе `in_progress`)
и пишет в `alert_messages`. `resolved_at` заполняется только при `resolved`/`closed` — это момент закрытия,
а не последнего касания.

Убедись, что статус реально сменился: `[OK] alerts <ids> → <status>` в выводе **и** `SELECT` выше.
Если после прогона обращение осталось в `in_progress` — проверь, не поставил ли ты `verdict` по инерции
там, где мяч на стороне пользователя (см. критерии в шаге 5).

При разборе старых `in_progress` не доверяй прошлым отчётам на слово: сверь, что у каждого письма есть
`unisender_id`. Отписанный или заблокированный адрес выпадает из отправки молча, и обращение годами
числится «отвеченным», пока человек ждёт ответа.
...
- Unisender `No valid recipients` (HTTP 400, code 204) = адрес отписан от рассылок. Ответ на прямое обращение
  человека — транзакционное письмо, а не рассылка, но обойти отписку через `skip_unsubscribe` нельзя:
  аккаунту нужен флаг `allow_skip_unsubscribe`, которого у нас нет (падает с code 1588). Пока флага нет —
  связываться вне email (телефон из заказа) и писать об этом в отчёте. Живой пример: alert #188, 31.08.2026.
