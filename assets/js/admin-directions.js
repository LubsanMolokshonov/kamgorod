/* Экономика направлений: inline-edit понедельных расходов + графики */

(function () {
    const data = window.DIR_DATA || {};

    // ============================================================
    // Inline сохранение расходов
    // ============================================================
    const inputs = document.querySelectorAll('.dir-cost-input');
    ...
    function saveCost(input) {
        ...
        fetch('/admin/directions/save-cost.php', {
            method: 'POST',
            body: formData,
            ...
            .then((res) => {
                ...
                if (res.success) {
                    ...
                    if (res.message) console.warn('Directions save error:', res.message);
                }
            })
            .catch((err) => {
                input.classList.remove('is-saving');
                input.classList.add('is-error');
                console.error('Directions save fetch error', err);
            });
    }
    ...
    const ctxRevCost = document.getElementById('dirChartRevenueCost');
    if (ctxRevCost) {
    ...
    const ctxProfit = document.getElementById('dirChartProfit');
    if (ctxProfit) {
        new Chart(ctxProfit, {
            type: 'line',
            data: {
