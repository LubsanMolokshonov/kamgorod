...
$reviewEntityName = $olympiad['title'] ?? '';
...
<?php
// CTA-баннер персонализируем названием олимпиады и детерминированно варьируем
// формулировку по её id — чтобы блок отличался между олимпиадами, но был стабилен.
$olTitleEsc = htmlspecialchars($olympiad['title'] ?? 'олимпиаду', ENT_QUOTES, 'UTF-8');
$olCtaSeed  = crc32('olympiad_cta:' . (int)$olympiad['id']);
$olCtaHeadings = [
    'Пройдите олимпиаду «' . $olTitleEsc . '» прямо сейчас',
    'Готовы пройти олимпиаду «' . $olTitleEsc . '»?',
    'Участвуйте в олимпиаде «' . $olTitleEsc . '» бесплатно',
];
$olCtaSubs = [
    '10 вопросов · бесплатно · результат сразу',
    'Проверьте знания по теме и оформите диплом для портфолио',
    'Несколько минут — и результат с дипломом уже в личном кабинете',
];
$olCtaHeading = $olCtaHeadings[$olCtaSeed % count($olCtaHeadings)];
$olCtaSub     = $olCtaSubs[$olCtaSeed % count($olCtaSubs)];
?>
<!-- ТЁМНЫЙ CTA-БАННЕР -->
<section class="rd-section tight">
  <div class="rd-wrap">
    <div class="rd-trust-band reveal" style="text-align:center;">
      <div class="rd-trust-head" style="max-width:none;">
        ...
        <h2 style="color:#fff;font:700 clamp(28px, 3vw, 40px) var(--font-sans);letter-spacing:-.02em;margin:0 0 12px;"><?php echo $olCtaHeading; ?></h2>
        <p style="color:#c2cdff;font-size:16px;line-height:1.55;margin:0 0 28px;"><?php echo $olCtaSub; ?></p>
