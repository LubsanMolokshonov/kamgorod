...
// FAQPage — тексты зеркалят видимый FAQ-блок внизу страницы
require_once __DIR__ . '/../includes/faq-helper.php';
$signupBonus = UserTokens::signupBonus();
$faqItems = [
    ['q' => 'Что такое токены и сколько их нужно?', 'a' => 'Токены — внутренняя валюта генератора. Один материал стоит 10–25 токенов в зависимости от типа. При регистрации дарим ' . $signupBonus . ' токенов — этого хватает на первый материал бесплатно. Дальше баланс пополняется пакетами.'],
    ['q' => 'Материалы соответствуют ФОП и ФГОС 2026?', 'a' => 'Да. Промпты выстроены под требования ФОП, ФАОП ОВЗ и ФГОС 2026: корректная структура, цели, УУД и этапы. Перед использованием материал всегда стоит проверить — это черновик, который вы доводите под себя.'],
    ['q' => 'В каком формате я получу материал?', 'a' => 'Техкарты, конспекты, тесты, классные часы и КТП — в DOCX, рабочие листы — в PDF, презентации — в PPTX. Все файлы готовы к печати и редактированию.'],
    ['q' => 'Чем адаптация отличается от генерации?', 'a' => 'Генерация создаёт материал с нуля по вашим параметрам. Адаптация берёт ваш готовый текст (чужой конспект, программу, техкарту) и переписывает его под нужные требования — например, упрощает под ОВЗ или переводит на другой класс.'],
    ['q' => 'Сколько времени занимает генерация?', 'a' => 'Обычно 20–40 секунд. Адаптация готового материала — 15–30 секунд. Файл сразу появляется в вашем кабинете.'],
    ['q' => 'Токены сгорают?', 'a' => 'Нет, купленные токены не имеют срока действия. Если генерация завершилась ошибкой, токены автоматически возвращаются на баланс.'],
];
...
$jsonLdArray = [$schema, buildFaqJsonLd($faqItems)];
