<?php
/**
 * Рассылка: запись вебинара «Как сохранить ресурс» + презентация + анкета + сертификат + следующий вебинар
 * Рассылается ТОЛЬКО зарегистрированным участникам вебинара.
 *
 * Запуск:
 *   php scripts/send_recording_resurs.php --test email@test.com   # тест одному
 *   php scripts/send_recording_resurs.php --dry-run               # проверка без отправки
 *   php scripts/send_recording_resurs.php                         # рассылка всем зарегистрированным
 */
...
// === Константы вебинара ===
$webinarId = 10;
$recordingUrl = 'https://clck.ru/3SM2uX';
$presentationUrl = 'https://clck.ru/3SGVsi';
$feedbackUrl = 'https://clck.ru/3SGi2j';
$nextWebinarTitle = 'Наставник 2026: новые подходы к сопровождению молодых педагогов';
$nextWebinarUrl = 'https://fgos.pro/vebinar/nastavnik-2026';
...
foreach ($users as $index => $user) {
    ...
    try {
        ...
        // Рендер HTML-шаблона
        ob_start();
        include BASE_PATH . '/includes/email-templates/webinar_recording_resurs.php';
        $htmlBody = ob_get_clean();

        // Plain text версия
        $textBody = "Здравствуйте, {$fullName}!\n\n";
        $textBody .= "5 марта состоялся вебинар «{$webinar_title}».\n";
        $textBody .= "Запись уже доступна — смотрите в любое удобное время!\n\n";
        $textBody .= "Смотреть запись: {$recordingUrl}\n\n";
        $textBody .= "Презентация и материалы от эксперта: {$presentationUrl}\n\n";
        ...
        $textBody .= "19 марта в 14:00 по Москве\n";
...
$logMessage = "[" . date('Y-m-d H:i:s') . "] RECORDING_RESURS | Mode: {$mode} | Total: {$totalUsers}, Sent: {$sent}, Skipped: {$skipped}, Failed: {$failed}\n";
