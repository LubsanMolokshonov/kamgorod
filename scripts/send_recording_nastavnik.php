<?php
/**
 * Рассылка: запись вебинара «Наставник 2026» + презентация + анкета + сертификат
 * Рассылается ТОЛЬКО зарегистрированным участникам вебинара.
 *
 * Запуск:
 *   php scripts/send_recording_nastavnik.php --test email@test.com   # тест одному
 *   php scripts/send_recording_nastavnik.php --dry-run               # проверка без отправки
 *   php scripts/send_recording_nastavnik.php                         # рассылка всем зарегистрированным
 */
...
// === Константы вебинара ===
$webinarId = 12;
$recordingUrl = 'https://clck.ru/3Sdwmg';
$presentationUrl = 'https://clck.ru/3SaKHd';
$feedbackUrl = 'https://clck.ru/3SaLJ4';
...
foreach ($users as $index => $user) {
    ...
    try {
        ...
        // Рендер HTML-шаблона
        ob_start();
        include BASE_PATH . '/includes/email-templates/webinar_recording_nastavnik.php';
        $htmlBody = ob_get_clean();

        // Plain text версия
        $textBody = "Здравствуйте, {$fullName}!\n\n";
        $textBody .= "19 марта состоялся вебинар «{$webinar_title}».\n";
        $textBody .= "Запись уже доступна — смотрите в любое удобное время!\n\n";
        $textBody .= "Смотреть запись: {$recordingUrl}\n\n";
        $textBody .= "Презентация и подарки от эксперта: {$presentationUrl}\n\n";
...
$logMessage = "[" . date('Y-m-d H:i:s') . "] RECORDING_NASTAVNIK | Mode: {$mode} | Total: {$totalUsers}, Sent: {$sent}, Skipped: {$skipped}, Failed: {$failed}\n";
