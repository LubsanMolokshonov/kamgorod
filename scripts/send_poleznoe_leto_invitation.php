<?php
/**
 * Массовая рассылка-приглашение на вебинар «Полезное лето для особого ребёнка»
 * (3 июня 2026) через Unisender Go (EmailDispatcher). Аудитория — горячая база
 * (таблица users): все, кто регистрировался на мероприятия fgos.pro.
 *
 * Копия send_webinar_invitation.php с собственными lock/pause-файлами, шаблоном
 * webinar_invitation_poleznoe_leto.php, темой и UTM этого вебинара.
 *
 * Режимы:
 ...
 *   --pause                 создать /tmp/poleznoe_leto_invitation.pause (cron остановится)
 *   --resume                удалить файл паузы
 *   --status                краткая статистика по логу
 *
 * Запуск (прод):
 *   docker exec pedagogy_web php /var/www/html/scripts/send_poleznoe_leto_invitation.php \
 *     --slug=poleznoe-leto-osobyj-rebenok --send --batch=100 --daily-cap=2500
 */
...
$LOCK  = '/tmp/poleznoe_leto_invitation.lock';
$PAUSE = '/tmp/poleznoe_leto_invitation.pause';
...
function sendOne(string $email, string $name, int $userId, array $webinar, string $webinar_date, string $webinar_datetime_full, bool $dryRun): array {
    ...
    $utm = 'utm_source=email&utm_medium=invite&utm_campaign=webinar-poleznoe-leto-jun2026';
    ...
    ob_start();
    include BASE_PATH . '/includes/email-templates/webinar_invitation_poleznoe_leto.php';
    $html = ob_get_clean();

    // нейтральная тема: без «бесплатно», «приглашаем», «акция», эмодзи и !!!.
    $subject = 'Полезное лето для особого ребёнка — вебинар ' . $webinar_date;
    ...
    $text .= "Разберём, как специалисту и семье вместе спланировать лето особого ребёнка так, чтобы каникулы стали ресурсом развития: как вовлечь даже «трудных» родителей, как говорить с разными типами семей. Только практика и готовые решения на реальных примерах.\n\n";
