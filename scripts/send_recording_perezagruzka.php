<?php
/**
 * Рассылка: запись вебинара «Перезагрузка отношений с родителями» (23.04.2026)
 * + презентация + анкета обратной связи + сертификат (magic-link)
 * Рассылается ТОЛЬКО зарегистрированным участникам вебинара.
 *
 * Запуск:
 *   php scripts/send_recording_perezagruzka.php --test email@test.com   # тест одному
 *   php scripts/send_recording_perezagruzka.php --dry-run               # проверка без отправки
 *   php scripts/send_recording_perezagruzka.php                         # рассылка всем зарегистрированным
 */
...
// === Константы вебинара ===
$webinarId = 17;
$recordingUrl = 'https://clck.ru/3TF7jR';
$presentationUrl = 'https://clck.ru/3TEyCL';
$feedbackUrl = 'https://clck.ru/3TEy7x';
...
foreach ($users as $index => $user) {
    ...
    try {
        ...
        ob_start();
        include BASE_PATH . '/includes/email-templates/webinar_recording_perezagruzka.php';
        $htmlBody = ob_get_clean();

        $textBody = "Здравствуйте, {$fullName}!\n\n";
        $textBody .= "23 апреля состоялся вебинар «{$webinar_title}».\n";
        ...
        $mail->isHTML(true);
        $mail->Subject = mb_encode_mimeheader($email_subject, 'UTF-8', 'B');
        $mail->Body = $htmlBody;
...
$logMessage = "[" . date('Y-m-d H:i:s') . "] RECORDING_PEREZAGRUZKA | Mode: {$mode} | Total: {$totalUsers}, Sent: {$sent}, Skipped: {$skipped}, Failed: {$failed}\n";
