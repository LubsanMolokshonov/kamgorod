<?php
/** Точечная замена согласованной редакции публикации №842. */
...
try {
    ...
    $slug = 'patrioticheskoe-vospitanie-cherez-kontsertno-ispolnitelskuyu-deyatelnost';
    ...
    if (!hash_equals($revision['approved_package_sha256'], $package['sha256'])) {
        throw new RuntimeException('Хеш комплекта не совпадает с подготовленной ревизией.');
    }
    ...
    $q = $db->prepare(
        'SELECT id, user_id, slug, source, status, title, annotation, content,
                ...
                cover_image_url, cover_status, format_status, updated_at
           FROM publications
          WHERE id = ? AND slug = ?
          FOR UPDATE'
    );
    ...
    if (!$row || $row['source'] !== 'upload' || $row['status'] !== 'published') {
    ...
    if (!$write) {
        $db->rollBack();
        echo "DRY-RUN OK: публикация 842; исходный SHA256 совпал; комплект проверен.\n";
    ...
    $backup = $backupDir . '/publication-842-' . gmdate('Ymd-His') . '-' . bin2hex(random_bytes(4)) . '.json';
    ...
    echo "Обновлена публикация 842. Резервная копия: {$backup}\n";
