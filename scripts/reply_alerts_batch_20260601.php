 ...
 *   docker exec pedagogy_web php /var/www/html/scripts/reply_alerts_batch_20260601.php            (DRY-RUN)
 *   docker exec pedagogy_web php /var/www/html/scripts/reply_alerts_batch_20260601.php --send      (живая отправка)
 *   docker exec pedagogy_web php /var/www/html/scripts/reply_alerts_batch_20260601.php --send --only=107,122  (точечно)
 */
...
echo "=== reply_alerts_batch_20260601.php [$mode] ===\n";
 ...
 */
$CASES = require __DIR__ . '/reply_alerts_batch_20260601_cases.php';
...
foreach ($CASES as $alertId => $case) {
    ...
    if (!empty($case['with_link'])) {
        $utm = ['utm_source'=>'email','utm_medium'=>'support','utm_campaign'=>'alert_reply_0601','utm_content'=>'alert_'.$alertId];
        if ($userId) {
    ...
    try {
        ...
        $res = EmailDispatcher::send([
            ...
            'meta'          => ['email_type'=>'other','touchpoint_code'=>'alert_reply_0601','user_id'=>$userId ?: null],
        ]);
        $sent++;
