#!/usr/bin/env python3
"""
Импорт курсов повышения квалификации (КПК, «СКОЛКОВО, 3 поток») из XLSX → SQL.

Вход:  курсы/КПК добавить на сайт.xlsx  (строки 2–32)
Выход: database/migrations/167_seed_kpk_courses_skolkovo.sql

Правила (согласованы с заказчиком):
- Пропускаем строки без title/hours/price (страховка).
- Пропускаем строки SKIP_ROWS = {22, 23, 25, 31, 32}:
  32 — нет outcomes и нет экспертов; 22/23/25/31 — нет экспертов.
- Итог: 26 курсов.
- Специализации сайдбара — по столбцу H «ЦА (для навигации на сайте)»;
  ЦА, у которых нет своего пункта фильтра, мапим на близкий существующий slug
  (новые пункты фильтра НЕ заводим).
...
Раскладка колонок КПК-xlsx (сдвиг на 1 против ПП):
  A=title  C=course_group  D=hours  F=learning_format  G=price
  H=ЦА  I=target_audience_text  J=description  K=modules  L=outcomes  M=experts
"""
...
XLSX_PATH = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    BASE_DIR, 'курсы', 'КПК добавить на сайт.xlsx')
OUTPUT_PATH = sys.argv[2] if len(sys.argv) > 2 else os.path.join(
    BASE_DIR, 'database', 'migrations', '167_seed_kpk_courses_skolkovo.sql')

# --- Строки xlsx, которые пропускаем целиком (решение заказчика) ---
SKIP_ROWS = {22, 23, 25, 31, 32}

# --- Эксперты, уже присутствующие в БД (full_name) — их НЕ вставляем повторно ---
# (снимок SELECT full_name FROM course_experts на 2026-09-03 после миграции 166)
EXISTING_EXPERTS = {
    ...
    # эксперты из миграций 100/110 и импортов 080/166 (по full_name)
    ...
    # эксперты из миграции 166 (КПК-xlsx)
    "Панина Татьяна Олеговна", "Омарова Патимат Омаровна",
    "Елецкая Ольга Вячеславовна", "Кишиневская Марина Александровна",
    "Белых Ксения Константиновна", "Дуванов Петр Петрович",
    "Колесниченко Любовь Ивановна", "Водопьянова Оксана Евгеньевна",
    "Лысова Ольга Юрьевна", "Таринова Наталья Владимировна",
    "Айдаров Юрий Рафаэлевич", "Шинакова Светлана Викторовна",
    "Рюмина Юлия Евгеньевна", "Букина Виктория Юрьевна",
    "Ишметова Марина Александровна", "Разгонюк Ольга Сергеевна",
    "Шутова Наталья Геннадьевна", "Ребковец Евгения Андреевна",
}

# --- Группа (столбец C) → audience_types.slug ---
GROUP_TO_TYPE_SLUGS = {
...
# --- ЦА (столбец H) → [(spec_slug, audience_type_id для пина)] ---
CA_TO_SPECS = {
    ...
    'Классный руководитель': [('klassnoe-rukovodstvo', 1)],
    'Социальный педагог': [('socialnaya-pedagogika', 1)],
    'Руководитель/заместитель руководителя': [('administratsiya-upravlenie', 1)],
    'Методист': [('metodist', 1)],
}
...
def parse_modules(text):
    ...
    for m in pattern.finditer(text):
        ...
        if title:
            items.append({'number': int(m.group(1)), 'title': title})
    # хвост «Итоговая аттестация», приклеенный к последнему модулю
    has_final = False
    if items:
        last = items[-1]['title']
...
def parse_outcomes(text):
    """Колонка L структурирована секциями «Знания» / «Умения» / «Навыки».
    Возвращает {"knowledge":[...],"skills":[...],"abilities":[...]}."""
    out = {'knowledge': [], 'skills': [], 'abilities': []}
    if not text:
        return out
    ...
    for ln in lines:
        if not ln:
            continue
        key = OUTCOME_HEADERS.get(ln.lower().rstrip(':').strip())
        if key is not None:
            cur = key
            continue
        if cur is None:
            # текст до первого заголовка — кладём в skills (fallback)
            cur = 'skills'
        item = ln.strip().rstrip(';').rstrip('.').strip()
        if item:
            out[cur].append(item)
    return out
...
def parse_experts(text):
    ...
    for blk in blocks:
        if not blk:
            continue
        mn = re.match(r'^([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){1,2})', blk[0].strip())
        if not mn:
            continue
...
def main():
    ...
    for rn in range(2, 200):
        if rn not in rows:
            continue
        rd = rows[rn]
        title = re.sub(r'\s+', ' ', g(rd, 'A')).strip()
        if not title:
            continue
        hours = parse_hours(g(rd, 'D'))
        price = parse_price(g(rd, 'G'))
        group = g(rd, 'C')
        slug = slugify(title)

        if rn in SKIP_ROWS:
            skipped.append((rn, title, 'исключено по решению заказчика (нет экспертов / нет outcomes)'))
            continue
        ...
        spec_pairs = []
        for token in [t.strip() for t in g(rd, 'H').split(',') if t.strip()]:
            if token not in CA_TO_SPECS:
                unknown_ca.add(f'row {rn}: {token!r}')
                continue
            for pair in CA_TO_SPECS[token]:
        ...
        courses.append({
            ...
            'slug': slug,
            'description': g(rd, 'J'),
            'target_audience_text': g(rd, 'I'),
            'course_group': course_group,
            'hours': hours,
            'learning_format': g(rd, 'F').rstrip('.').strip() or
                               'заочная с применением дистанционных образовательных технологий',
            'price': price,
            'modules': parse_modules(g(rd, 'K')),
            'outcomes': parse_outcomes(g(rd, 'L')),
            'type_slugs': type_slugs,
            'spec_pairs': spec_pairs,
            'experts': parse_experts(g(rd, 'M')),
        })
    ...
    # новые эксперты (по slug, не в БД)
    new_experts = {}
    for c in courses:
        for e in c['experts']:
    ...
    # ---------- генерация SQL ----------
    L = []
    L.append('-- Migration 167: Seed KPK courses «СКОЛКОВО, 3 поток»')
    L.append('-- Auto-generated by scripts/import_kpk_courses_skolkovo.py')
    L.append('--   из "курсы/КПК добавить на сайт.xlsx".')
    L.append('-- Идемпотентна: INSERT IGNORE по уникальным slug.')
    ...
    for n, c in enumerate(courses, 1):
        ...
        L.append(
            ...
            f"'kpk', {sql_str(c['learning_format'])}, {c['price']}, "
    ...
    # ---------- отчёт ----------
    print(f'OK  {len(courses)} курсов -> {OUTPUT_PATH}', file=sys.stderr)
    print(f'OK  {len(new_experts)} новых экспертов', file=sys.stderr)
    print(f'SKIP {len(skipped)} строк:', file=sys.stderr)
    for s in skipped:
        print(f'   xlsx-строка {s[0]}: {s[1][:70]} ({s[2]})', file=sys.stderr)
    if unknown_ca:
        print('!! НЕОПОЗНАННЫЕ ЦА-токены:', file=sys.stderr)
        for u in sorted(unknown_ca):
    ...
    for c in courses:
        ...
        print(f"        группа={c['course_group']!r} часы={c['hours']} цена={c['price']} "
              f"модулей={len(c['modules'])} экспертов={len(c['experts'])} "
              f"outcomes(з/у/н)={len(c['outcomes']['knowledge'])}/{len(c['outcomes']['skills'])}/{len(c['outcomes']['abilities'])}",
              file=sys.stderr)
