<?php
/**
 * Рассылка: запись вебинара «Нейросети для педагога» + презентация + анкета + сертификат + следующий вебинар
 * Рассылается ТОЛЬКО зарегистрированным участникам вебинара.
 *
 * Запуск:
 *   php scripts/send_recording_nejroseti.php --test email@test.com   # тест одному
 *   php scripts/send_recording_nejroseti.php --dry-run               # проверка без отправки
 *   php scripts/send_recording_nejroseti.php                         # рассылка всем зарегистрированным
 */
...
// === Константы вебинара ===
$webinarId = 14;
$recordingUrl = 'https://clck.ru/3SyMrY';
$presentationUrl = 'https://clck.ru/3SyMgU';
$feedbackUrl = 'https://clck.ru/3SxoUx';
$nextWebinarTitle = '«Дружить» или не «дружить»? Перезагрузка отношений с родителями: от конфликтов к доверию';
$nextWebinarUrl = 'https://fgos.pro/vebinar/perezagruzka-otnoshenij-s-roditelyami';
...
foreach ($users as $index => $user) {
    ...
    try {
        ...
        // Рендер HTML-шаблона
        ob_start();
        include BASE_PATH . '/includes/email-templates/webinar_recording_nejroseti.php';
        $htmlBody = ob_get_clean();

        // Plain text версия
        $textBody = "Здравствуйте, {$fullName}!\n\n";
        $textBody .= "7 апреля состоялся вебинар «{$webinar_title}».\n";
        $textBody .= "Запись уже доступна — смотрите в любое удобное время!\n\n";
        $textBody .= "Смотреть запись: {$recordingUrl}\n\n";
        $textBody .= "Презентация (в неё вшиты полезные материалы): {$presentationUrl}\n\n";
        ...
        $textBody .= "23 апреля в 14:00 по Москве\n";
...
$logMessage = "[" . date('Y-m-d H:i:s') . "] RECORDING_NEJROSETI | Mode: {$mode} | Total: {$totalUsers}, Sent: {$sent}, Skipped: {$skipped}, Failed: {$failed}\n";
