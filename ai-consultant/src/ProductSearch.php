...
class ProductSearch
{
    ...
    public function search(string $query, int $limit = 8): array
    {
        ...
        // Если ничего релевантного не нашлось — возвращаем пустой список, чтобы
        // не подсовывать пользователю случайные товары того же типа.
        if ($productType !== null) {
            return $this->searchTable($productType, $tableMap[$productType], $keywords, $audienceSlugs, $limit);
        }
     ...
     * Каждое слово приводится к грубой «основе» — отрезаются типичные русские
     * падежные/числовые окончания, чтобы LIKE '%stem%' матчил разные формы
     * («логопедов» → «логопед», «переподготовки» → «переподготовк»).
     */
    private function extractKeywords(string $query): array
    {
        $stopWords = [
            ...
            'какой','какая','какое','какие','кто','кому','где','когда','куда','зачем','почему',
            'такой','такая','такое','такие','весь','вся','все','всё','более','менее','очень','ещё',
            'или','либо','тоже','также','бы','ли','то','же','про','об','обо','со','во',
        ];
    ...
    /**
     * Грубая стемминг-эвристика для русского: отрезает часто встречающиеся
     * падежные/числовые окончания. Не лингвистически корректно, но достаточно
     * для поиска через LIKE '%stem%'.
     */
    private function stemRu(string $word): string
    {
        $suffixes = [
            'ами','ями','ого','его','ему','ому','ыми','ими','ыми','ться','тся',
            'ая','яя','ое','ее','ый','ий','ой','ей','ую','юю','ые','ие','их','ых',
            'ам','ям','ах','ях','ов','ев','ёв','ём','ом','ем','ей','ой','ою','ею',
            'ал','ял','ил','ел','ел','ла','ло','ли','ут','ют','ат','ят','ет','ит',
            'ть','ла','ло','ли','ся','сь','ка','ко','ки','ек',
            'а','я','о','е','ы','и','у','ю','й','ь',
        ];
        // Сортируем по длине, чтобы сначала пытаться отрезать более длинные окончания
        usort($suffixes, fn($a, $b) => mb_strlen($b) - mb_strlen($a));
        foreach ($suffixes as $suf) {
            $len = mb_strlen($word);
            $sufLen = mb_strlen($suf);
            if ($len - $sufLen >= 4 && mb_substr($word, -$sufLen) === $suf) {
                return mb_substr($word, 0, $len - $sufLen);
            }
        }
        return $word;
    }
